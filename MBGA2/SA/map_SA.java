...
public class map_SA {
	...
	public void infoC(int col, int row, Mapm m, Timestamp ini) {
		...
		for (int i = 0; i < n.getInfoFlights().size(); i++) {
			if (n.getInfoFlights().get(i).getInitialD().before(ini) && n.getInfoFlights().get(i).getLastD().after(ini)) {
			Flight f = n.getInfoFlights().get(i).getFlight();
			s = s+ "Origen: "+ f.getSource() + "(" + parseDate(f.getDeparture_time()) + ")\n"+"Destino: "+ f.getDestination() + "(" + parseDate(f.getArrival_time()) + ")\n"+"Estado: "+f.getPlane_state()+"\n"+"--------------"+"\n";
			}
		}
		if (s!="") {
