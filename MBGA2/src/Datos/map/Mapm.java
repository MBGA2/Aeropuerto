...
public class Mapm {
	...
	public void freezeFlight(Flight f,Timestamp t) {
		int x = -1, y = -1;
		if (this.flightList.containsKey(f)) {
			for (Iterator<Tuple<Integer,Integer,Timestamp>> iterator = this.flightList.get(f).iterator(); iterator.hasNext();) {
			...
			this.infoMap.get(x+","+y).delayFlight(f, 20160, true); //Se queda en el sitio 2 semanas o hasta llegada de SAR
		}
	}
